...
export function resolveExcelDirectory(): string {
...
function listSourceFiles(excelDir: string): string[] {
  ...
  if (!files.length) {
    throw new HttpError(
      500,
      `No source files found in ${excelDir}. Expected .xlsx/.xls/.csv files for buyer seed import.`
    );
  }
  return files;
}

export function buildExcelSignature(excelDir: string, files: string[]): string {
...
export function dedupeRows(rows: CanonicalRow[]): CanonicalRow[] {
...
export function buyerIdFromRow(row: CanonicalRow): string {
  const buyerKey = [
    row.productName,
    row.casNo || '',
    row.buyerName,
  ...
  return `buyer-${buyerHash}`;
}

export function rowToBuyer(row: CanonicalRow): BuyerMasterModel {
  const saltId = `salt-${slugify(row.productName)}`;
  const medicineId = `med-${slugify(row.productName)}`;
  return {
    id: buyerIdFromRow(row),
    medicineId,
    saltId,
...
export function parseExcelDirectory(excelDir = resolveExcelDirectory()): {
  excelDir: string;
  sourceFiles: string[];
  rows: CanonicalRow[];
  buyers: BuyerMasterModel[];
} {
  const sourceFiles = listSourceFiles(excelDir);
  ...
  return { excelDir, sourceFiles, rows, buyers };
}

export function buildMasterDataFromBuyers(
  buyers: BuyerMasterModel[],
  meta: { sourceDirectory: string; sourceFiles: string[] }
): MasterDataModel {
  ...
  for (const buyer of buyers) {
    const productKey = buyer.productName.toLowerCase();

    if (!saltsByProduct.has(productKey)) {
      saltsByProduct.set(productKey, {
        id: buyer.saltId,
        name: buyer.productName,
    ...
    if (buyer.casNo) salt.casNumbers.add(buyer.casNo);
    salt.sourceFiles.add(buyer.sourceFile);
    salt.buyerNames.add(buyer.buyerName);
    if (buyer.companyCategory) salt.companyCategories.add(buyer.companyCategory);
    if (buyer.country) salt.countries.add(buyer.country);
    for (const cert of buyer.certifications) {
      salt.certifications.add(cert);
    }
    if (buyer.annualBuyingCapacityKg !== null) {
      salt.totalAnnualBuyingCapacityKg += buyer.annualBuyingCapacityKg;
    }

    if (!medicine.casNumber && buyer.casNo) medicine.casNumber = buyer.casNo;
    medicine.sourceFiles.add(buyer.sourceFile);
    medicine.buyerNames.add(buyer.buyerName);
    if (buyer.annualBuyingCapacityKg !== null) {
      medicine.totalAnnualBuyingCapacityKg += buyer.annualBuyingCapacityKg;
    }
  }
...
export function buildMasterDataFromExcel(excelDir = resolveExcelDirectory()): MasterDataModel {
  const parsed = parseExcelDirectory(excelDir);
  return buildMasterDataFromBuyers(parsed.buyers, {
    sourceDirectory: parsed.excelDir,
